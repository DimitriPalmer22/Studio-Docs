There are 9 journal entries in the game. Each of them has the following:
- A set of dialogue for when the journal table spawns in your room
- The text of the journal entry itself ([[]])
- A set of dialogue that appears immediately after reading the journal entry
- A line of dialogue for when the journal entry's objective is complete


# Stuff

### Journal Entry + Research Notes = Bad
- It's kinda weird to try to fit both of these within the same note
	- Gets too long
	- Kinda hard to stay in character

### Locking Progression Behind Random Elements Might not be Fun If they're sequential
- yeah


### New Progression Idea

Two different types of pages:
- Journal pages that are just text
- Experiment logs that have the conditions for completion

### Idea for Narratively Justifying Progression
The pages and stuff will have like runes on them or something so that they indicate in the game world "Hey this is how we open the seal on the door fr"


# Journal Pool 1
5 Pages
- 3 journal entries
- 2 experiment log

### Journal Entries


### Experiment Logs


# Journal Pool 2
10 Pages
- 6 journal entries
- 4 experiment logs

### Journal Entries


### Experiment Logs


# Journal Pool 3
15 Pages
- 9 journal entries
- 6 experiment logs

### Journal Entries


### Experiment Logs


# Visual Progression
The door at the end of Avernoth has a magical seal on it. There are runes all around it. As you collect the entries or whatever, they appear.

Each journal entry / experiment log has a rune on it that corresponds to one of the runes on the sealed door.

!![](<../../../_Meta/Attachments/Pasted image 20251118154456.png>)


# Condition Modifiers
- Current Area of Avernoth
- Enemy defeated count (in save )